...
// - 登录态持久化（userId + token）由 UserState.onLoginSuccess 负责
//   （userId 写入 DataStore；token 由 UserPreferences 委托给 EncryptedPrefs，决策 4）
// - 退出登录清理由 UserState.logout 负责
// ============================================================================
 ...
 *
 * 通用登录能力，业务方拉取脚手架后：
 * 1. 修改 [login] 方法为真实后端登录接口（用 [safeApiCall] 包装）
 * 2. 添加 `register` / `refreshToken` 等方法
 * 3. 删除「模拟登录」逻辑（包括下面的 `email.contains("fail")` 模拟失败路径）
 *
 * 当前实现：模拟登录（无真实后端）
 * - email 含 "fail" 时返回 [ApiResult.Error]（业务异常），让 UI 的 Error 分支可被验证（Medium #17）
 * - 否则构造虚拟 [UserEntity] 返回成功，通过 [UserDao] 持久化用户基本信息
 * - 不再持久化登录态（token / userId）—— 该职责已迁移到
 *   [com.pai.app.navigation.UserState.onLoginSuccess]
 *
 * 设计要点：
 * - 用 [safeApiCall] 包装网络请求返回 [ApiResult]
 * - 调用方（如 [com.pai.app.feature.auth.AuthViewModel]）在登录成功后
 *   调用 `userState.onLoginSuccess(user.id, token)` 完成 DataStore + EncryptedPrefs 持久化
 */
@Singleton
class AuthRepository @Inject constructor(
     ...
     * - 否则构造虚拟用户返回成功，写入 [UserDao] 持久化用户信息
     * - 不写 DataStore / EncryptedPrefs（由调用方 [com.pai.app.navigation.UserState.onLoginSuccess] 处理）
     *
     * 业务方改造示例：
     * ```kotlin
     * suspend fun login(email: String, password: String): ApiResult<UserEntity> {
     *     val result = safeApiCall { api.login(LoginRequest(email, password)) }
     *     if (result is ApiResult.Success) {
     *         val user = result.data.toEntity()
     *         userDao.upsert(user)
     *     }
     *     return result
     * }
     ...
     * @return 成功返回虚拟用户，失败返回 [ApiResult.Error]
     */
    internal suspend fun login(email: String): ApiResult<UserEntity> {
        // 模拟网络延迟
        delay(500)
